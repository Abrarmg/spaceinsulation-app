...
import { 
  ...
  Phone, Mail, Navigation, Wrench, FileText, Check, Trash2, Camera, Upload, User, Clock, Package
} from 'lucide-react';
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 text-[#64748B] text-sm font-semibold">
                ...
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider ${getStatusColor(job.status)}`}>
                  {job.status}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E2E8F0] p-4 md:p-5">
            ...
            {job.customer_signature_url ? (
              <div className="bg-[#ECFDF5] rounded-xl border border-[#A7F3D0] p-4 text-center">
                ...
                <div className="bg-white rounded-lg p-2 border border-[#A7F3D0] inline-block">
                  <img src={job.customer_signature_url} alt="Customer Signature" className="h-16 object-contain" />
                </div>
              </div>
            ) : (
...
os.makedirs('src/pages/jobs', exist_ok=True)
with open('src/pages/jobs/WorkerJobDetailView.tsx', 'w') as f:
    f.write(content)
